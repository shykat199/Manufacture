...
@section('page-breadcrumb','Product List')
        ...
        <div class="table-wrapper">
            <table id="datatable1" class="table display responsive nowrap">
                ...
                <tr>
                    ...
                    <th class="wd-15p">Product Warehouse</th>
                ...
                <tbody>
                @foreach($getProductLists as $key => $product)

                    <tr>
                        <td>{{++$key}}</td>
                        <td>{{$product->products->name}}</td>
                        <td>{{$product->productSize->productSize}}</td>

                        <td>{{$product->products->price}}</td>
                        <td>{{$product->product_qty}}</td>
                        <td>{{$product->product_total_price}}</td>
                        <td>{{$product->productWareHouse->name}}</td>

                        <td>{{formatedCreateAt($product->created_at)}}</td>
                        <td>
                            <div class="button-group">
                                <a href="{{route('admin.warehouse.product.details',[$product->id])}}" class="btn btn-secondary"><i class="fa-solid fa-pen-to-square"></i></a>
        ...
        function deleteUser(id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let userDltRoute = '{{route('admin.warehouse.product.delete',['id'=>':id'])}}'
